...
const useZoomStore = create(
  immer<IZoomStore>(set => ({
    ...
    setReferenceLineX (index, top) {
      set(draft => {
        draft.referenceLinesX[index].top = top
      })
    },
    clearReferenceLineX (index, key) {
      set(draft => {
        draft.referenceLinesX = draft.referenceLinesX.filter(
          (item, i) => !(item.key === key && i === index)
        )
      })
    },
    addReferenceLineY () {
    ...
    setReferenceLineY (index, left) {
      set(draft => {
        draft.referenceLinesY[index].left = left
      })
    },
